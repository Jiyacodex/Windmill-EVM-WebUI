...
export default function CodeBlock({ code, className, label }: CodeBlockProps) {
  ...
  return (
    <div className={cn('relative group flex flex-col', className)}>
      {label && (
        <span className="text-[11px] font-semibold text-neutral-600 dark:text-neutral-400 mb-1.5 flex items-center gap-1.5">
      ...
      <div className="relative">
        <div className="bg-neutral-50 dark:bg-neutral-800/80 border border-neutral-100 dark:border-neutral-700 rounded-xl p-4 font-mono text-xs text-black dark:text-white overflow-x-auto">
          <pre className="whitespace-pre pr-16">{code}</pre>
        </div>
        <button
          type="button"
          onClick={handleCopy}
